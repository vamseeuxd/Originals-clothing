...
export class BrandService {
  private brandsAction: AngularFirestoreCollection<IBrand>
  readonly brands$: Observable<IBrand[]>
  ...
  constructor(
    ...
    this.brandsAction = firestore.collection<IBrand>('brands')
    this.brands$ = firestore
      .collection<IBrand>('brands', (ref) => {
  ...
  deleteBrand(brand: IBrand): Promise<any> {
    return new Promise(async (resolve, reject) => {
      const busyIndicatorId = this.busyIndicator.show()
      try {
        const docRef = await this.brandsAction.doc(brand.id)
